...
import { Settings, Trash2, CheckCircle, AlertCircle, MessageSquare } from 'lucide-react';
      ...
      <div className={`px-4 py-3 rounded-t-lg flex items-center justify-between ${
        ...
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
            ...
            <MessageSquare className={`w-5 h-5 ${data.isConfigured ? 'text-purple-600' : 'text-gray-500'}`} />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900">{data.label}</h3>
            <p className="text-xs text-gray-500">Configure responses</p>
          </div>
        </div>
        ...
        {data.isConfigured ? (
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {data.configData?.comment_content && (
                <div className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded">
                  Comment: "{data.configData.comment_content.substring(0, 30)}..."
                </div>
              )}
              {data.configData?.dm_content && (
                <div className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded">
                  DM: "{data.configData.dm_content.substring(0, 30)}..."
                </div>
              )}
            </div>
            <div className="text-xs text-gray-500">
              Response messages configured
            </div>
          </div>
        ) : (

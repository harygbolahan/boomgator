...
import { Settings, Trash2, CheckCircle, AlertCircle, Cog } from 'lucide-react';
import { Button } from '@/components/ui/button';

const ConfigNode = ({ data, id }) => {
      ...
      <div className={`px-4 py-3 rounded-t-lg flex items-center justify-between ${
        ...
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
            ...
            <Cog className={`w-5 h-5 ${data.isConfigured ? 'text-purple-600' : 'text-gray-500'}`} />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900">{data.label}</h3>
            <p className="text-xs text-gray-500">Final settings</p>
          </div>
        </div>
        ...
        {data.isConfigured ? (
          <div className="space-y-2">
            <div className="text-sm text-gray-600">
              <span className="font-medium">Label:</span> {data.configData?.label || 'Not set'}
            </div>
            <div className="text-xs text-gray-500">
              {data.configData?.titles?.length > 0 && (
                <div>Titles: {data.configData.titles.length} configured</div>
              )}
              {data.configData?.urls?.length > 0 && (
                <div>URLs: {data.configData.urls.length} configured</div>
              )}
            </div>
            <div className="text-xs text-gray-500">

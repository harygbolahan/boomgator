...
import { Settings, Trash2, CheckCircle, AlertCircle, User } from 'lucide-react';
import { Button } from '@/components/ui/button';

const PageNode = ({ data, id }) => {
      ...
      <div className={`px-4 py-3 rounded-t-lg flex items-center justify-between ${
        ...
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
            ...
            <User className={`w-5 h-5 ${data.isConfigured ? 'text-purple-600' : 'text-gray-500'}`} />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900">{data.label}</h3>
            <p className="text-xs text-gray-500">Select page/account</p>
          </div>
        </div>
        ...
        {data.isConfigured ? (
          <div className="space-y-2">
            <div className="text-sm text-gray-600">
              <span className="font-medium">Page:</span> {data.configData?.pageName || 'Configured'}
            </div>
            <div className="text-xs text-gray-500">
              {data.configData?.pageDescription || 'Page configuration completed'}
            </div>
          </div>
        ) : (
